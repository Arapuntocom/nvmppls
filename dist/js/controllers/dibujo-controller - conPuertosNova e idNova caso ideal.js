...
.controller('DibujoController', function($scope, $timeout, $mdSidenav, $log, $mdDialog, $document, contextMenu, $mdMenu, $rootScope, $compile) {
	...
	var crearEnlace =  function(){
		...
		var cellOrigen = graph.getCell(estacionOrigen.id);
		var cellDestino = graph.getCell(estacionDestino.id);
		var idNovaDestino, idNovaOrigen;
		var cantEnlacesSalientesConectados = 0;

		idNovaOrigen = cellOrigen.get('etiquetas').idNova;

		var arrayPuertosOrigen = cellOrigen.getPorts();
		$scope.perro = arrayPuertosOrigen[0].group;
		for(var i =0; i<arrayPuertosOrigen.length; i++){
			if(arrayPuertosOrigen[i].group == 'out')
				cantEnlacesSalientesConectados++;
		}
		$log.debug("cantEnlacesSalientesConectados -> "+cantEnlacesSalientesConectados);

		if(cellOrigen.get('type') == 'basic.CicloConversacional'){ //si origen es ciclo conversacional, entonces agrega Letra
			idNovaDestino = String.fromCharCode(64+cantEnlacesSalientesConectados);
		}else{ //origen es estacion de control, entonces agrega Numero
			idNovaDestino = cantEnlacesSalientesConectados;
		}		
		
		if(idNovaOrigen == 'Main'){
			idNovaDestino = ''+idNovaDestino;
		}else{
			idNovaDestino = ''+idNovaOrigen+idNovaDestino;
		}

		$log.debug("idNovaDestino-> "+idNovaDestino);
		
		cellDestino.set('attrs',{'.idNova':{text: idNovaDestino}});
		cellDestino.set('etiquetas',{idNova: idNovaDestino});
